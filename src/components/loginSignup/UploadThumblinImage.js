...
class UploadThumblinImage extends Component {
    constructor( props ) {
        super( props );
    }
    state = {
    ...
    handleUploadThumblinImage() {
        this.setState( { onFocus: 'VideoIntro' } )
        alert( "handleUploadThumblinImage" )
    }
    handleDoItLater() {
        alert( "Do it later" )
    }
    ...
    handleNextClick() {
        ...
        // this.props.navigation.replace( 'HomeScreen', { profileType: 'VIP' } );
        this.props.navigation.replace( 'AccountSettings', { profileType: 'VIP' } );
    }
    render() {
        return (
            <View style={[styles.container, { backgroundColor: this.props.theme.primaryBackgroundColor }]}>
                ...
                <View style={styles.contentContainer}>
                    <TouchableOpacity
                        onPress={() => this.handleUploadThumblinImage()}
                        ...
                        <CustomComponents.CustomText value={strings.UPLOAD_THUMBLIN_IMAGE} style={{ fontSize: 16 }} />
...
export default connect( mapStatetoProps, mapDispatchToProps )( UploadThumblinImage );
